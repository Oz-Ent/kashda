...
const NewSavingsForm: React.FC<NewSavingsFormProps> = ({
  onClose,
  onCreatePlan,
}) => {
  ...
  const handleCreatePlan = () => {
    ...
    const goalId = `${planName
      .toLowerCase()
      .replace(/\s+/g, "-")}-${Date.now()}`;

    // Determine category based on plan name (simple heuristic)
    const getCategory = (name: string): string => {
      const lowerName = name.toLowerCase();
      if (lowerName.includes("emergency") || lowerName.includes("rainy day")) {
        return "Emergency";
      } else if (
    ...
    // Create the goal object matching the structure in appData.json
    const newGoal = {
      id: goalId,
      name: planName.trim(),
      targetAmount: toNumber(targetAmount),
      currentAmount: toNumber(initialDeposit) || 0,
      targetDate: endDate,
      category: getCategory(planName),
    };

    console.log("Creating plan with goal:", newGoal);

    // Call the onCreatePlan callback to append the goal to the local state
    if (onCreatePlan) {
      onCreatePlan(newGoal);
    } else {
      console.error("onCreatePlan callback is not provided");
    }
  ...
  const periodsBetween = (
    ...
    const diffDays = Math.max(
      0,
      Math.ceil((endMid.getTime() - startMid.getTime()) / msPerDay)
    );
    switch (frequency) {
      case "Daily":
        return diffDays;
      ...
      case "Monthly": {
        let months =
          (endMid.getFullYear() - startMid.getFullYear()) * 12 +
          (endMid.getMonth() - startMid.getMonth());
      ...
      <div className="px-2">
        {summary ? (
          ...
          <>
            ...
            <form
              className="h-[27rem] w-full flex flex-col gap-3 px-1 overflow-y-auto"
              ...
              <div className="flex flex-col gap-2 mr-3">
                ...
                {endDateManuallyEdited && selectedFrequency !== "Manual" && (
                  <span className="text-xs text-[#a0a0a0]">
                    Target amount is auto-calculated from end date.
                  </span>
                )}
              </div>
              <div className="flex flex-col gap-2 mr-3">
                ...
                {targetManuallyEdited && selectedFrequency !== "Manual" && (
                  <span className="text-xs text-[#a0a0a0]">
                    End date is auto-calculated from target amount.
                  </span>
                )}
              </div>
              <p className="text-[#ffa500] text-md">
                Funds are deducted from usable funds
              </p>
              <p className="text-[#c72626] text-md">
                Disclaimer: Exiting a plan early reduces earned interest by 45%.
              </p>
              <div className="flex items-center justify-center">
                <button
                  onClick={(e) => {
                    ...
                    if (!selectedFrequency) {
                      alert("Please select a deduction frequency");
                      return;
                    }
                    if (
                      selectedFrequency !== "Manual" &&
                  ...
                  type="button"
                  className="w-[80%] h-[3rem] bg-[#c19813] hover:bg-[#e6c24d] text-white px-4 py-2 mb-3 rounded-lg flex items-center justify-center space-x-2 transition-colors duration-200 cursor-pointer"
                >
                  Proceed to Summary
                </button>
              </div>
